...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-path', default='./save/proto-1_attention2_1gammma_version5_learable=False')
    ...
    filename = 'final_lr=0.0001_Allinfoweight_attention2_1gamma_version5_learable=False_1shot.csv'
    column_names = ['gamma1','gamma2','changegamma1','changegamma2','lr1','accuracy',
                    'gamma11','gamma21','changegamma11','changegamma22','lr2','accuracy2']
    with open(filename, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(column_names)
        convolutional_network=resnet18(pretrained=True)
        convolutional_network.fc=nn.Flatten()
        weight=[1,1.1,1.2,1.3,1.4]
        model_true=PrototypicalNetworks(convolutional_network,weightlearnable=False,weight=weight).cuda()
        ...
        torch.save(model_true.state_dict(), osp.join(args.save_path, f'newprototypical_true' + '.pth'))
        ...
        convolutional_network=resnet18(pretrained=True)
        convolutional_network.fc=nn.Flatten()
        model_false=PrototypicalNetworks(convolutional_network,weightlearnable=False,weight=weight).cuda()
        ...
        torch.save(model_false.state_dict(), osp.join(args.save_path, f'newprototypical_false' + '.pth'))
        
        values = [1,1,model_true.gamma1.item(),model_true.gamma2.item(),0.0001,result_after_true,
                        1,1,model_false.gamma1.item(),model_false.gamma2.item(),0.001,result_after_false]
